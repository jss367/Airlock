...
airlock focus [-h|--help] [--ignore-floating] [--wrap-around]
                [--boundaries <boundary>] [--boundaries-action <action>]
                (dfs-next|dfs-prev)
airlock focus [-h|--help] [--ignore-floating]
...
https://github.com/jss367/Airlock/issues/5
...
(app-next|app-prev|same-app-next|same-app-prev)::
Cycle focus through windows of applications.
`app-next` and `app-prev` cycle focus between different applications within the current workspace.
`same-app-next` and `same-app-prev` cycle through windows of the same application within the current workspace.
This mode is incompatible with `--wrap-around`, `--boundaries`, and `--boundaries-action`.
