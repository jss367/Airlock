...
Airlock is in early development. Download the release ZIP from https://github.com/jss367/Airlock/releases/latest[GitHub Releases] for Apple Silicon or Intel Macs running macOS 13 or later. Extract it, move `Airlock.app` to `/Applications`, and copy `bin/airlock` to a directory on your `PATH`, such as `~/.local/bin`. Releases are ad-hoc signed and are not notarized by Apple; macOS may require you to explicitly allow the app before opening it.

Alternatively, build it from a checkout of https://github.com/jss367/Airlock[the repository] using Xcode with Swift 6.2 or later:
...
To install and launch the app, run `./deploy.sh`. This stops any running Airlock instance, rebuilds and tests, replaces `/Applications/Airlock.app`, installs `~/.local/bin/airlock`, and launches the app. Add `~/.local/bin` to your `PATH` to use the command-line tool.

Grant Airlock access in *System Settings → Privacy & Security → Accessibility* when prompted. See https://github.com/jss367/Airlock/blob/main/dev-docs/development.md[development instructions] for setup and build options.

[#workspace-isolation]
== Workspace isolation and navigation

Airlock keeps app and window switching within the focused workspace. With the default configuration:

* `cmd-tab` and `shift-cmd-tab` cycle applications in the current workspace.
* `cmd-backtick` and `shift-cmd-backtick` cycle windows of the same app in that workspace.
* `option-space` opens the quick switcher for the current workspace.
* `option-1` through `option-9` switch to the corresponding workspace.

Airlock also blocks attempts by apps on other workspaces to steal focus. Use an explicit workspace command or the workspace overview to move between workspaces.

The menu bar provides *Mission Control*, *Show Keybindings*, and *Keyboard Visualizer*. The keyboard visualizer can edit bindings in your configuration. For app launching and summoning, see xref:commands.adoc#app-launcher[app-launcher] and xref:commands.adoc#summon-app[summon-app].
...
[mode.main.binding]            # Declare 'main' binding mode
    option-r = 'mode resize'      # 1. Define a binding to switch to 'resize' mode
...
----
[mode.main.binding]
    # Bind option-1 key to switch to workspace 1
    option-1 = 'workspace 1'
    # Or bind a sequence of commands
    option-shift-1 = ['move-node-to-workspace 1', 'workspace 1']
----
...
[mode.main.binding]
    option-unicorn = 'workspace wonderland' # (⁀ᗢ⁀)
----
...
You can configure environment variables of `exec-*` commands and callbacks (such as xref:commands.adoc#exec-and-forget[exec-and-forget])
...
Airlock manages its own workspaces rather than using native macOS Spaces. This lets it control window placement and switch layouts directly.
...
Once you switch back to the workspace, (for example, with the xref:commands.adoc#workspace[workspace] command) windows are placed back to the visible area of the screen.
...
Airlock workspaces are separate from native macOS Spaces. When troubleshooting focus or window placement across monitors, include the value of *Displays have separate Spaces* in your bug report, along with your monitor arrangement and macOS version.

Airlock also checks the macOS setting *When switching to an application, switch to a Space that has open windows for the application* at startup. If it is enabled, the app offers to disable it because automatic native Space switching can interfere with Airlock's workspace management. Accepting that prompt restarts the Dock.
...
After a workspace switch or other cross-context jump it can be hard to
spot where focus just landed. Airlock can briefly draw a pulsing outline
around the focused window to make this obvious.

It is enabled by default with `mode = 'cross-workspace'`, which means it
fires on workspace switches but stays quiet during normal in-workspace
navigation. To turn it off entirely:
...
The `mode` key controls when the auto-flash fires. Available modes:

* `every` -- on every focus change
* `cross-workspace` -- only when the focused workspace changes (default)
* `cross-app` -- only when the focused app changes
* `idle` -- only after `idle-threshold-seconds` of no focus change
* `off` -- never auto-flash (the xref:commands.adoc#flash-focus[flash-focus] command still works)

Color, line width, pop distance, and animation duration are all
configurable -- see the `[focus-flash]` table in the <<default-config>>.

You can also flash on demand by binding the
xref:commands.adoc#flash-focus[flash-focus] command to a hotkey:

[source,toml]
----
[mode.main.binding]
    option-shift-f = 'flash-focus'
----
